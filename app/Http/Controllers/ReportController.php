...
class ReportController extends Controller
{
    ...
    public function buku_besar(Request $request)
    {
        ...
        if($dir == 'hutang_supplier'){
            ...
            $data['jk'] = Detail_jurnal_pengeluaran_kas::select('tanggal_jurnal','keterangan',DB::raw('cast(sub_total as decimal(65,2)) as debet'))->whereBetween('tanggal_jurnal', [$from, $to])->Where('kode_akun_debit', $kode)->orderBy('tanggal_jurnal','DESC')->get();
        
            $data['report'] = Detail_jurnal_umum::select('tanggal_jurnal', 'keterangan',DB::raw('cast(sub_total as decimal(65,2)) as debet'))->whereBetween('tanggal_jurnal', [$from, $to])->Where('kode_akun_debit', $kode)->orderBy('tanggal_jurnal','DESC')->get();
